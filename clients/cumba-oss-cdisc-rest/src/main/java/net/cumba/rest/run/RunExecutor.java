...
import net.cumba.cdisc.core.run.CancelledException;
import net.cumba.cdisc.core.run.StudyValidationResult;
import net.cumba.rest.config.CorejProperties;
import net.cumba.rest.report.ReportStore;
import net.cumba.rest.report.RunLog;
import net.cumba.rest.report.RunLogAssembler;
    ...
    private void persistRuleDefsQuietly(CheckRun run, StudyValidationResult result)
    {
        try
        {
            reportStore.persistRuleDefs(run.id(),
                    net.cumba.rest.report.RuleDefsAssembler.assemble(result));
        }
        catch (Exception e)
        {
