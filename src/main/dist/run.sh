#!/bin/sh
# Convenience launcher for the coreJ CLI distribution.
#
# Resolves its own directory so the bundle is fully relocatable: the sidecar
# cumba-oss-corej-cli.conf and every ${bootstrap.dir} reference inside it resolve
# against the bundle, not against the caller's working directory.
#
...
# below becomes `java -jar /cumba-oss-corej-cli.jar`, whose error names a path that was never involved.
src=$0
while [ -L "$src" ]; do
...
exec java ${JAVA_OPTS} -jar "$DIR/cumba-oss-corej-cli.jar" "$@"
