...
This directory ships **empty**, for the same reason as `rules/` beside it: the
corpus is released separately.

It is needed only by `-vx` / `--validate-xml` (Define-XML conformance). A run that
does not use `-vx` never reads this directory.

Get it from the matching release of
[cumba-oss-corej-rules](https://github.com/cumba-oss/cumba-oss-corej-rules) —
asset `cumba-oss-corej-rules-define-<version>.zip`, which is a **separate asset**
from the data corpus — and move its contents here:
